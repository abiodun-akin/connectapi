...
const publishEvent = async (exchange, eventType, data) => {
  try {
    if (!process.env.RABBITMQ_URL) {
      console.warn(
        `[RabbitMQ] Skipping publish for ${eventType} — RABBITMQ_URL not set`,
      );
      return;
    }
    if (!channel) {
      console.warn(
        `[RabbitMQ] Channel not ready for ${eventType}, attempting reconnect...`,
      );
      await initializeRabbitMQ();
    }
    if (!channel) {
      console.error(
        `[RabbitMQ] Channel unavailable — could not publish ${eventType}`,
      );
      return;
    }
