...
const applySchema = {
  motivation: (value) => {
    if (!value || typeof value !== "string" || value.trim().length < 20) {
      throw new ValidationError("Motivation must be at least 20 characters", "motivation");
    }
  },
};
...
router.post("/apply", validateRequest(applySchema), async (req, res, next) => {
  try {
...
router.post("/withdrawals", validateRequest(withdrawalSchema), async (req, res, next) => {
  try {
    ...
    const pending = await AgentWithdrawal.findOne({ agent_id: req.user._id, status: "pending" });
    if (pending) {
